...
t1_sorted<- t1 %>% as.data.frame() %>% arrange(factor(Var1, levels = c("Stereolithography", "Extrusion based", "Inject based", "Other", "Unclear")))
...
#  colnames<-(t(t13_sorted[-1]), t13_sorted[,1]), 
 # 'labels' <- (t(t13_sorted[-1]), t13_sorted[,1])), 
  # t(as.matrix(t13_sorted)), 
   data_bar1,
  ylab = "Number of Studies", ylim=c(0, max(data_bar1) + 10), cex.names=.8 , col = "green", main = "What kind of printing method was used?"), y = data_bar1 + 2, labels = data_bar1, 
  cex = 0.8)
...
#  colnames<-(t(t13_sorted[-1]), t13_sorted[,1]), 
 # 'labels' <- (t(t13_sorted[-1]), t13_sorted[,1])), 
  # t(as.matrix(t13_sorted)), 
   data_bar4,
  ylab = "Number of Studies", ylim=c(0, max(data_bar4) + 10), cex.names=.8 , col = "green", main = "What kind of forms are printed with this ink?"), y = data_bar4 + 2, labels = data_bar4, 
  cex = 0.8)
...
t16 <- table(sorted$cellDensity)

#t16

text(x = barplot(t16, ylab = "Number of Inks", ylim=c(0, max(t16) + 10), cex.names=.8 , col = "grey", main = "What is the Reported Cell Density"), y = t16 + 2, labels = t16, cex = 0.8)
...
text(x = barplot(t8, ylab = "Number of BioInks", ylim=c(0, max(t8) + 10), cex.names=0.8 , col = "blue", main = "What is the origin of BioInk"), y = t8 + 2, 
     labels = t8, cex = 0.8)
...
formattable(liverMarkers,
            ...
            list(`Indicator Name` = formatter(
              "span", style = ~ style(color = "grey",font.weight = "bold")), 
              `n_studies_report`= color_bar("yellow")
))
